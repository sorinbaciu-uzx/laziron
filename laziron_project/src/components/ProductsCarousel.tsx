...
export function ProductsCarousel({
  ...
  // On phones we show one full-width card at a time (paged via swipe/arrows),
  // so the continuous auto-scroll is disabled to keep the card centered.
  useEffect(() => {
  ...
  useEffect(() => {
    if (single || reduced || paused || isMobile) return;
    let raf = 0;
  ...
  }, [single, reduced, paused, isMobile]);
        ...
        {products.map((product) => (
          <div
            ...
            className={`shrink-0 snap-center sm:snap-start ${
              single ? "w-full max-w-sm" : "w-full sm:w-80 lg:w-[22rem]"
            }`}
          >
